 ...
 *          contlegsA:    list of contracted legs in A (row vector)      %int32!!!
 *          contlegsB:    list of contracted legs in B (row vector)   the ordering should match the one in contlegsA     %int32!!!
 *          freelegsA:    list of free (not contracted) legs in A (row vector). Supposed to be in ascending order.       %int32!!!
 *          freelegsB:    list of free (not contracted) legs in B (row vector). Supposed to be in ascending order.       %int32!!!
...
void tensordot(double *Areal, double *Aimag, double *Breal, double *Bimag, double *Creal, double *Cimag, int32_t *contlegsA, int32_t *contlegsB, int32_t *freelegsA, int32_t *freelegsB, 
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    int32_t *contlegsA;          /* array of contracted legIDs in A */
    int32_t *contlegsB;          /* array of contracted legIDs in B */
    int32_t *freelegsA;          /* array of free legIDs in A */
    int32_t *freelegsB;          /* array of free legIDs in B */
    ...
    /* Check if inputs 3rd-6th inputs are int32 row vectors */
    if((mxGetM(prhs[2]) > 1) ||
       (mxGetM(prhs[3]) > 1) ||
       (mxGetM(prhs[4]) > 1) ||
       (mxGetM(prhs[5]) > 1) ||
       ...
       (!mxIsInt32(prhs[4])) ||
       (!mxIsInt32(prhs[5]))){
        mexErrMsgIdAndTxt("MyToolbox:tensordot:nrhs","contlegsA, contlegsB, freelegsA, freelegsB must be int32 row vectors");  
    }

    contlegsA = (int32_t *) mxGetData(prhs[2]);
    contlegsB = (int32_t *) mxGetData(prhs[3]);
    freelegsA = (int32_t *) mxGetData(prhs[4]);
    freelegsB = (int32_t *) mxGetData(prhs[5]);
