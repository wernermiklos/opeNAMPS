...
wigner2 = 0;
for t = tmin:2:tmax
    wigner1 = wigner1 + (-1)^t / ( prod(1:t) * prod(1:(t-t1)) * prod(1:(t-t2)) ...
        * prod(1:(t3-t)) * prod(1:(t4-t)) * prod(1:(t5-t)) );
end

for t = (tmin+1):2:tmax
    wigner2 = wigner2 + (-1)^t / ( prod(1:t) * prod(1:(t-t1)) * prod(1:(t-t2)) ...
        * prod(1:(t3-t)) * prod(1:(t4-t)) * prod(1:(t5-t)) );
end

wigner = (wigner1 + wigner2) * (-1)^(j1-j2-m3) ...
    * sqrt( prod(1:(j1+j2-j3)) * prod(1:(j1-j2+j3)) * prod(1:(-j1+j2+j3)) / prod(1:(j1+j2+j3+1))...
        * prod(1:(j1+m1)) * prod(1:(j1-m1)) * prod(1:(j2+m2)) * prod(1:(j2-m2)) * prod(1:(j3+m3)) * prod(1:(j3-m3)) );
