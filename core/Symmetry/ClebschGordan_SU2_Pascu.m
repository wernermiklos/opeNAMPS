...
function C = ClebschGordan_SU2_Pascu(j1,m1,j2,m2,j,m)
...
%---compute coefficient---%
C = sqrt((2*j+1)*prod(1:j+j1-j2)*prod(1:j+j2-j1)*prod(1:j1+j2-j)/prod(1:j1+j2+j+1))*...
    sqrt(prod(1:j+m)*prod(1:j-m)*prod(1:j1+m1)*prod(1:j1-m1)*prod(1:j2+m2)*prod(1:j2-m2))*...
    sum(((-1).^k)./(prod(1:k).*prod(1:j1+j2-j-k).*prod(1:j1-m1-k).*prod(1:j2+m2-k).*prod(1:j-j2+m1+k).*prod(1:j-j1-m2+k)));