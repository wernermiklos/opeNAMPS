...
%   center_pos:         the center position (the "the last leg of the left part")
    ...
    j = 0;
    ops_are_scalar = false;

    if ~isempty(varargin)
        if strcmp(varargin{1},'Scalar')    %All ops are scalar in the coupling ==> no 'M' legs!
            ops_are_scalar = true;
        end
    end
    if no_of_sites == 1
        ops_are_scalar = true;          % If all ops are on the same site ==> op must be a scalar!
    end
    ...
    if ~isempty(eta_groups{i})
        ...
        new_eta_names{i} = cell(1,length(eta_groups{i}));
        for k = 1:length(eta_groups{i})
            new_eta_names{i}{k} = eta_groups{i}{k}(1:(end-2));
        end
        Coupling_FULL = NTrename_legs(Coupling_FULL,eta_groups{i},new_eta_names{i});
    end
